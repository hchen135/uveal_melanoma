...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, net, predictions, features, targets, masks, num_crowds, semantic_seg_annotated_mask, UM_class):
        """Multibox Loss
        Args:
        ...
        UM_class_pred_data = predictions['UM_class_pred'] if 'UM' in cfg.name and cfg.apply_classification else None
        ...
        pure_negative_ind = []#images that has no positive objects
        for idx in range(batch_size):
            if len(targets[idx]) == 0:
                pure_negative_ind.append(idx)
                conf_t[idx,:] = -1
                continue
        ...
        if cfg.train_masks:
            if cfg.mask_type == mask_type.direct:
                ...
                ret = self.lincomb_mask_loss(pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, pure_negative_ind)
                if cfg.use_maskiou:
        ...
        if cfg.use_semantic_segmentation_loss:
            losses['S'] = self.semantic_segmentation_loss(predictions['segm'], masks, labels, semantic_seg_annotated_mask, pure_negative_ind)
       
        # ADDED!!!
        if cfg.use_discriminative_loss:
            losses['R'] = self.discriminative_loss(features,masks,cfg.delta_var,cfg.delta_dist,cfg.discriminative_norm)
        if 'UM' in cfg.name and cfg.apply_classification:
            losses['U'] = nn.CrossEntropyLoss()(UM_class_pred_data,torch.stack(UM_class))
        # Divide all losses by the number of positives.
        # Don't do it for loss[P] because that doesn't depend on the anchors.
        total_num_pos = num_pos.data.sum().float()
        for k in losses:
            if k not in ('P', 'E', 'S', 'U', 'R'):
    ...
    def semantic_segmentation_loss(self, segment_data, mask_t, class_t, semantic_seg_annotated_mask, pure_negative_ind, interpolation_mode='bilinear'):
        ...
        for idx in range(batch_size):
            ...
            cur_semantic_seg_annotated_mask = semantic_seg_annotated_mask[idx]

            if idx in pure_negative_ind:# if the image is purely negative, all pixels are background
                segment_t = torch.zeros_like(cur_segment, requires_grad=False)
                downsampled_semantic_seg_annotated_mask = torch.ones_like(cur_segment, requires_grad=False)
            else:
                with torch.no_grad():
                    ...
                    #new
                    downsampled_semantic_seg_annotated_mask = F.interpolate(semantic_seg_annotated_mask[idx].unsqueeze(0), (mask_h, mask_w),
                                                      mode=interpolation_mode, align_corners=False).squeeze(0)
                    downsampled_semantic_seg_annotated_mask = downsampled_semantic_seg_annotated_mask.gt(0.5).float()
            ...
            #new: only annotated part of the image.    
            loss_candidate = F.binary_cross_entropy_with_logits(cur_segment, segment_t, reduction='none')
            loss_s += torch.sum(loss_candidate*downsampled_semantic_seg_annotated_mask.float())
            count += torch.sum(downsampled_semantic_seg_annotated_mask).float()

        return loss_s / count * cfg.semantic_segmentation_alpha
    ...
    def discriminative_loss(self,features,masks,delta_var,delta_dist,norm,interpolation_mode='bilinear'):
        #'''
        loss_fn = DiscriminativeLoss(delta_var,delta_dist,norm)
        batch_size = features.shape[0]
        _, mask_h, mask_w = masks[0].size()
        downsampled_masks = []
        n_objects = []
        for idx in range(batch_size):
            n_objects.append(masks[idx].shape[0])

        upsampled_features = F.interpolate(features, (mask_h, mask_w),
                                                  mode=interpolation_mode, align_corners=False).squeeze(0)
        # bs, n_instances, h, w
        n_instances = max(n_objects)
        formulated_masks = torch.zeros(batch_size,n_instances,mask_h,mask_w,requires_grad=False)
        for idx in range(batch_size):
            formulated_masks[idx,:n_objects[idx],:,:] = masks[idx]
        loss = loss_fn(upsampled_features,formulated_masks,n_objects,n_instances)
        return loss
        #'''
        return 0
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, pure_negative_ind, interpolation_mode='bilinear'):
        ...
        for idx in range(mask_data.size(0)):
            if idx in pure_negative_ind:
                maskiou_t_list.append([])
