...
class Compose(object):
    ...
    def __call__(self, img, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        for t in self.transforms:
            img, masks, boxes, labels, semantic_annotated_mask = t(img, masks, boxes, labels, semantic_annotated_mask)
        return img, masks, boxes, labels, semantic_annotated_mask
...
class Lambda(object):
    ...
    def __call__(self, img, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        return self.lambd(img, masks, boxes, labels, semantic_annotated_mask)


class ConvertFromInts(object):
    def __call__(self, image, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        return image.astype(np.float32), masks, boxes, labels, semantic_annotated_mask
...
class ToAbsoluteCoords(object):
    def __call__(self, image, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        ...
        return image, masks, boxes, labels, semantic_annotated_mask
...
class Pad(object):
    ...
    def __call__(self, image, masks, boxes=None, labels=None, semantic_annotated_mask=None):
        ...
        if self.pad_gt:
            ...
            masks = expand_masks
            if not semantic_annotated_mask is None:
                expand_semantic_annotated_mask = np.zeros(
                    (1, self.height, self.width),
                    dtype = semantic_annotated_mask.dtype)
                expand_semantic_annotated_mask[:,:im_h,:im_w] = semantic_annotated_mask
                semantic_annotated_mask = expand_semantic_annotated_mask

        return expand_image, masks, boxes, labels, semantic_annotated_mask
...
class Resize(object):
    ...
    def __call__(self, image, masks, boxes, labels=None, semantic_annotated_mask=None):
        ...
        if self.resize_gt:
            ...
            semantic_annotated_mask = semantic_annotated_mask.transpose((1,2,0))
            semantic_annotated_mask = cv2.resize(semantic_annotated_mask, (width, height))
            
            # OpenCV resizes a (w,h,1) array to (s,s), so fix that
            if len(semantic_annotated_mask.shape) == 2:
                semantic_annotated_mask = np.expand_dims(semantic_annotated_mask, 0)
            else:
                semantic_annotated_mask = semantic_annotated_mask.transpose((2, 0, 1))
...
class ToCV2Image(object):
    def __call__(self, tensor, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        return tensor.cpu().numpy().astype(np.float32).transpose((1, 2, 0)), masks, boxes, labels, semantic_annotated_mask


class ToTensor(object):
    def __call__(self, cvimage, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        return torch.from_numpy(cvimage.astype(np.float32)).permute(2, 0, 1), masks, boxes, labels, semantic_annotated_mask
...
class RandomSampleCrop(object):
    ...
    def __call__(self, image, masks, boxes=None, labels=None, semantic_annotated_mask=None):
        ...
        while True:
            ...
            for _ in range(50):
                current_image = image
                current_semantic_annotated_mask = semantic_annotated_mask
                ...
                current_image = current_image[rect[1]:rect[3], rect[0]:rect[2],
                                              :]
                current_semantic_annotated_mask = current_semantic_annotated_mask[:,rect[1]:rect[3], rect[0]:rect[2]]
                ...
                return current_image, current_masks, current_boxes, current_labels, current_semantic_annotated_mask
...
class Expand(object):
    ...
    def __call__(self, image, masks, boxes, labels, semantic_annotated_mask=None):
        if random.randint(2):
            return image, masks, boxes, labels, semantic_annotated_mask
        ...
        if not semantic_annotated_mask is None:
            expand_semantic_annotated_mask = np.zeros(
                 (1,int(height*ratio), int(width*ratio)),
                 dtype=image.dtype)
            expand_semantic_annotated_mask[:,int(top):int(top + height),
                         int(left):int(left + width)] = semantic_annotated_mask
            semantic_annotated_mask = expand_semantic_annotated_mask
...
class RandomMirror(object):
    def __call__(self, image, masks, boxes, labels, semantic_annotated_mask=None):
        ...
        if random.randint(2):
            ...
            if not semantic_annotated_mask is None:
                semantic_annotated_mask = semantic_annotated_mask[:,:,::-1]
        return image, masks, boxes, labels, semantic_annotated_mask
...
class RandomRot90(object):
    def __call__(self, image, masks, boxes, labels, semantic_annotated_mask=None):
        ...
        if not semantic_annotated_mask is None:
            semantic_annotated_mask = np.array([np.rot90(mask,k) for mask in semantic_annotated_mask])
        boxes = boxes.copy()
        for _ in range(k):
...
class PhotometricDistort(object):
    ...
    def __call__(self, image, masks, boxes, labels, semantic_annotated_mask=None):
        im = image.copy()
        im, masks, boxes, labels, semantic_annotated_mask = self.rand_brightness(im, masks, boxes, labels, semantic_annotated_mask)
        if random.randint(2):
            distort = Compose(self.pd[:-1])
        else:
            distort = Compose(self.pd[1:])
        im, masks, boxes, labels, semantic_annotated_mask = distort(im, masks, boxes, labels, semantic_annotated_mask)
        return self.rand_light_noise(im, masks, boxes, labels, semantic_annotated_mask)
...
class PrepareMasks(object):
    ...
    def __call__(self, image, masks, boxes, labels=None, semantic_annotated_mask=None):
        ...
        return image, new_masks, boxes, labels, semantic_annotated_mask
...
class BackboneTransform(object):
    ...
    def __call__(self, img, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        ...
        return img.astype(np.float32), masks, boxes, labels, semantic_annotated_mask
...
class BaseTransform(object):
    ...
    def __call__(self, img, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
        return self.augment(img, masks, boxes, labels, semantic_annotated_mask)
...
def do_nothing(img=None, masks=None, boxes=None, labels=None, semantic_annotated_mask=None):
    return img, masks, boxes, labels, semantic_annotated_mask
...
class SSDAugmentation(object):
    ...
    def __call__(self, img, masks, boxes, labels, semantic_annotated_mask):
        return self.augment(img, masks, boxes, labels, semantic_annotated_mask)
