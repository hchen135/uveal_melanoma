...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        # only for discriminative loss
        self.dimension_reduce_conv = nn.Conv2d(256,16,1)

        if 'UM' in cfg.name and cfg.apply_classification:
            self.UM_classification_layer = nn.Sequential(
                                               nn.Conv2d(256,cfg.num_classes,3,padding=1),
                                               nn.AvgPool2d(int(64*0.5**cfg.backbone.selected_layers[0]))
                                                        )
    ...
    def forward(self, x):
        ...
        if 'UM' in cfg.name and cfg.apply_classification:
            pred_outs['UM_class_pred'] = self.UM_classification_layer(outs[self.proto_src]).squeeze(2).squeeze(2)
        ...
        if self.training:
            ...
            return pred_outs, self.dimension_reduce_conv(outs[self.proto_src])
        else:
            if cfg.use_mask_scoring:
            ...
            return self.detect(pred_outs, self), outs[self.proto_src]
