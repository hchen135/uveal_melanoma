...
class BOA(object):
    ...
    # If maxlen is big, fpgrowh tendås to generate too many rules that overflow the memories. 
    ...
    def TP_gene(self,df,indices,criterion='<='):
        data = []
        len_rules = []
        variable_ind = []
        for rule in indices:
            len_rules.append(rule.count(criterion))
            for ind in range(len(rule)//3):
                if rule[int(ind*3 + 1)] == criterion:
                    variable_ind.append(rule[int(ind*3)])
                    data.append(rule[int(ind*3 + 2)])
        data = np.array(data)
        variable_ind = np.array(variable_ind).astype(int)
        indptr =list(accumulate(len_rules))
        indptr.insert(0,0)
        indptr = np.array(indptr)
        indiceMatrix = csc_matrix((np.ones_like(data),variable_ind,indptr),shape = (len(df.columns),len(indices)))
        ruleMatrix = csc_matrix((data,variable_ind,indptr),shape = (len(df.columns),len(indices)))
        #because matrix can only be 2d, we need to for loop samples to get the entire mat matrix
        mat = np.zeros((df.shape[0],len(indices)))
        for ind in range(df.shape[0]):
            sample_use = df.values[ind,:].reshape(-1,1)
            sample_use = csc_matrix(indiceMatrix.multiply(sample_use))
            if criterion == '<=':
                mat_single = sample_use < ruleMatrix # WARNING!!! USING < instead of <= to speed up sparse matrix operation!!!#########################
            elif criterion == '>':
                mat_single = sample_use > ruleMatrix
            else:
                print("WRONG OPERATION IN TREE")
                assert 0
            mat_single = indiceMatrix.multiply(mat_single)
            mat_single = np.sum(mat_single,axis=0)
            mat[ind] = mat_single
        '''
        df_use = df.reshape(df.shape[0],df.shape[1],1)
        mat = df_use <= ruleMatrix
        mat = np.multiply(mat,indiceMatrix)
    ...
    def screen_rules(self,rules,df,N):
        ...
        for i,name in enumerate(df.columns):
            itemInd[name] = i
        indices = [[itemInd[x] if i%3 == 0 else x for i,x in enumerate(rule)] for rule in rules]# change the names of rules to indices
        
        mat_smaller = self.TP_gene(df,indices,'<=')
        mat_larger  = self.TP_gene(df,indices,'>' )

        mat = mat_smaller + mat_larger
        len_rules = [len(rule)//3 for rule in rules]
        lenMatrix = np.matrix([len_rules for i in range(df.shape[0])])
        Z =  (mat ==lenMatrix).astype(int)
        print(Z.shape,self.Y.shape)
    ...
    def SA_patternbased(self, Niteration = 5000, Nchain = 3, q = 0.1, init = [], print_message=True):
        ...
        self.rules_len = [len(rule)//3 for rule in self.rules]
    ...
    def propose(self, rules_curr,rules_norm,q):
        ...
        if len(move)>0 and move[0]=='clean':
            remove = []
            for i,rule_1 in enumerate(rules_norm):
                Yhat = (np.sum(self.RMatrix[:,[rule_2 for j,rule_2 in enumerate(rules_norm) if (rule_2!=rule_1 and rule_2 not in remove)]],axis = 1)>0).astype(int)
                TP,FP,TN,FN = getConfusion(Yhat,self.Y)
                if TP+FP==0:
                    remove.append(rule_1)
            for x in remove:
                rules_norm.remove(x)
    ...
    def predict(self,rules,df):
        ...
        lenMatrix = np.matrix([len_rules for i in range(df.shape[0])])
        Z =  (mat ==lenMatrix).astype(int)
        Yhat = (np.sum(Z,axis=1)>0).astype(int)
        return Yhat
...
def extract_rules(tree, feature_names):
    ...
    def recurse(left, right, child, lineage=None):          
        ...
        if child in left:
            ...
            suffix = '<='
            thresh = threshold[int(parent)]
        else:
            parent = np.where(right == child)[0].item()
            suffix = '>'
            thresh = threshold[int(parent)]

        lineage.extend([thresh,suffix,features[parent].strip()])
